// Netlify function — manually trigger a compliance reminder email
// Called from the "Email me now" button in the dashboard.
// POST /.netlify/functions/compliance-reminder-manual
// Body: JSON array of compliance records (taken directly from the browser)
//
...
<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden;">
  ...
  <div style="background:#1d4ed8;padding:20px 24px;">
    <h1 style="color:#fff;margin:0;font-size:18px;">🏠 Berkeley PM — Compliance Status</h1>
  ...
  <div style="padding:20px 24px;">
    <div style="display:flex;gap:12px;margin-bottom:20px;">
      ...
      <div style="flex:1;background:#fffbeb;border:1px solid #fcd34d;border-radius:8px;padding:12px;text-align:center;">
        ...
        <div style="font-size:11px;color:#92400e;">Upcoming (60d)</div>
      </div>
      <div style="flex:1;background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:12px;text-align:center;">
        <div style="font-size:24px;font-weight:700;color:#16a34a;">${complete}/${total}</div>
        <div style="font-size:11px;color:#15803d;">Complete</div>
      </div>
    </div>
  `;
  ...
  if (!overdue.length && !upcoming.length) {
    html += `<p style="color:#6b7280;text-align:center;padding:20px;">✅ No upcoming or overdue items right now.</p>`;
  }

  html += `
    <p style="font-size:12px;color:#9ca3af;border-top:1px solid #f3f4f6;padding-top:12px;margin-top:4px;">
      Sent from your Berkeley PM Dashboard · <a href="https://ericpropertymanager.netlify.app" style="color:#3b82f6;">Open Dashboard →</a>
    </p>
  </div>
</div>
</body></html>`;
...
exports.handler = async (event) => {
  ...
  if (!apiKey) {
    return { statusCode: 500, body: JSON.stringify({ error: 'RESEND_API_KEY not configured in Netlify environment variables' }) };
  }
  ...
  const subject = overdue.length
    ...
    : `📅 Compliance status: ${upcoming.length} upcoming deadline${upcoming.length !== 1 ? 's' : ''} — Berkeley PM`;
